...
class Sparse_attention(nn.Module):
    ...
    def forward(self, attn_s):
        ...
        if time_step <= self.top_k:
            ...
            # normalize
        #print(attn_s.shape,delta.shape, delta.repeat(time_step).shape, time_step)
        ...
        #print(attn_w_sum.shape)
        attn_w_normalize = attn_w / attn_w_sum.repeat(1, time_step).view(attn_w.shape)
...
class SAB_LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, truncate_length=100,
                 block_attn_grad_past=False, print_attention_step=1, attn_every_k=1, top_k=5):
    ...
    def forward(self, x):
        ...
        for i, input_t in enumerate(x.chunk(time_size, dim=1)):
            ...
            h_repeated = h_t.clone().unsqueeze(1).repeat(1, remember_size, 1)
